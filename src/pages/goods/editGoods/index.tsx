...
class AddGoods extends Component<AddGoodsProps> {
  ...
  handleSubmit = async e => {
    ...
    params.goodsId = this.state.good_info.good_id;
    const { dispatch } = this.props;
    let status;

    if (dispatch) {
      status = await dispatch({
        type: 'good/updateGoods',
        payload: params,
      });
    }

    if (status.data.code === 0) {
      message.info('更新商品成功，可以返回商品页查看');
  ...
  render() {
    ...
    if (this.state && this.state?.good_info) {
      const { good_img, good_info, good_name, good_num, good_price } = this.state.good_info;
      return (
        <PageHeaderWrapper content="你可以在当前页面编辑商品">
          <Form {...layout} name="addGoods" onSubmit={this.handleSubmit}>
            <Form.Item label="商品名称" name="goodsName">
              {getFieldDecorator('goodsName', {
                initialValue: good_name,
